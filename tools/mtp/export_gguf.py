...
def orient(val: np.ndarray, gguf_shape: tuple) -> np.ndarray:
    """Match the trained tensor to the original gguf layout (auto-transpose 2D)."""
    if tuple(val.shape) == gguf_shape:
        return val
    if val.ndim == 2 and tuple(val.shape) == gguf_shape[::-1]:
        return np.ascontiguousarray(val.T)
    raise ValueError(f"shape mismatch: trained {val.shape} vs gguf {gguf_shape}")
    ...
    for t in reader.tensors:
        ...
        if param is not None and param in trained:
            ...
            if qtype == GGMLQuantizationType.F32:
                ...
                writer.add_tensor(
                    t.name, q, raw_dtype=qtype, raw_shape=list(gguf_shape)
                )
            n_patched += 1
        else:
