...
def decode_tps_of(g: dict) -> float | None:
    """gamut JSON shape may evolve; check the obvious places."""
    for key in ("decode_tps", "decodeTps"):
        if key in g and isinstance(g[key], (int, float)):
            return float(g[key])
    headline = g.get("headline") or g.get("metrics") or {}
    for key in ("decode_tps", "tps_decode", "tps"):
        v = headline.get(key)
...
def per_kernel_growth(sweep: dict[int, dict]) -> dict[str, dict[int, float]]:
    """Per-kernel mean time (us) at each ctx, if present in the gamut JSON."""
    out: dict[str, dict[int, float]] = {}
    for ctx, g in sweep.items():
        kerns = g.get("kernels") or g.get("per_kernel") or []
        if isinstance(kerns, dict):
            kerns = [{"name": k, **v} for k, v in kerns.items()]
        for k in kerns:
            name = k.get("name") or k.get("slug")
            t_us = k.get("mean_us") or k.get("total_us") or k.get("time_us")
