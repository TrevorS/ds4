...
[[ -n "$PROMPT_FILE" && -f "$PROMPT_FILE" ]] || { echo "need --prompt-file" >&2; exit 2; }
...
# Per-frontier gamut.py: ingest the captured artifacts, emit JSON sidecar.
...
for mtp_tag in nomtp mtp; do
    for ctx in "${FRONTIER_LIST[@]}"; do
        ...
        csv="$OUT_DIR/$slug.csv"
        [[ -f "$csv" ]] || continue
        # ds4_bench.c:480 — ctx_tokens,prefill_tokens,prefill_tps,gen_tokens,gen_tps,kvcache_bytes
        read -r PREFILL DECODE KVBYTES < <(awk -F, 'NR==2 {print $3, $5, $6}' "$csv")
        KVMB=$(awk -v b="${KVBYTES:-0}" 'BEGIN {printf "%.1f", b/1048576}')
        "$HERE/../gamut.py" \
            --plain "$OUT_DIR/$slug.plain.sqlite" \
            --metrics "$OUT_DIR/$slug.metrics.sqlite" \
            --ptxas "$PTXAS" \
